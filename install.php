...
<title>HRW Cms© ~ Installation</title>
<link rel=\"shortcut icon\" href=\"favicon.ico\" type=\"image/vnd.microsoft.icon\" />
<link rel=\"stylesheet\" href=\"web-gallery/v2/styles/install.css\" type=\"text/css\" />

</head>
<body>
<h1 id=\"logo\"><img alt=\"BioCMS\" src=\"housekeeping/images/holocms-logo.png\" /></h1>\n";

///////////////////////////////////////////////////////////////////////////////////////////////

if($step == 0){
echo "<p><strong>Bienvenue dans l'installation de HRW Cms©</strong></p>\n";
echo "<p>Bienvenue sur HRW Cms©. Avant de pouvoir commencer &agrave; utiliser votre nouveau site Web, nous avons besoin de mettre en place un certain nombre de choses. Pour que l'installation soit r&eacute;ussie, vous aurez besoin de conna&icirc;tre toutes les choses suivantes.</p>\n";
echo "<ol>\n";
echo "	<li>Nom de la base de donn&eacute;es MySQL</li>\n";
echo "	<li>Nom d'utilisateur MySQL</li>\n";
echo "	<li>Mot de passe MySQL</li>\n";
echo "	<li>MySQL h&ocirc;te (si ce n'est pas sur localhost)</li>\n";
echo "<p><b>Pour cette installation, pour mener &agrave; bien, le script d'installation doit avoir acc&egrave;s &agrave; &eacute;crire sur config.php. Pour obtenir de l'aide &agrave; donner les permissions d'&eacute;criture ou de la connexion &agrave; votre base de donn&eacute;es MySQL, s'il vous pla&icirc;t consulter votre h&ocirc;te / ISP.</b></p>\n";
echo "<p>S'il vous pla&Icirc;t soyez conscient que ce programme d'installation remplace (et donc vide) les tables du CMS existantes (et n&eacute;cessaire) dans votre base de donn&eacute;es MySQL, et &eacute;ventuellement modifier holographe Emulator tables en cas de besoin. S'il vous pla&Icirc;t lire les informations de compatibilit&eacute; ci-dessous pour optimiser les performances et l'int&eacute;gration.</p>\n";
echo "<p><strong>Version HRW Cms© :</strong> V 2.0<br />\n<strong>Date de Construction de HRW Cms© :</strong> Mars 2010<br />\n<strong>Holograph Emulator Compatability:</strong> Build for <a href='http://trac2.assembla.com/holograph/changeset/".$holograph['revision']."' target='_blank'>Revision ".$holograph['revision']."</a> (".$holograph['type'].")</p>\n";
echo "<p><a href=\"install.php?step=1\" class=\"button\">Nouvelle installation</a>\n&nbsp;<a href=\"upgrade.php\" class=\"button\">Mise à Jour</a></p>\n";
...
if(isset($_POST['name'])){
	...
	if(!$write){
		...
		if (!$db_connect) {
			$message = "Impossible de se connecter au serveur MySQL. S'il vous pla&icirc;t v&eacute;rifiez le nom d'h&ocirc;te, nom d'utilisateur et votre mot de passe que vous avez fournis.";
		} elseif (!$db_select) {
			$message = "Impossible de s&eacute;lectionner la base de donn&eacute;es. S'il vous pla&icirc;t v&eacute;rifiez le nom de base de donn&eacute;es que vous avez fournies.";
		} else {
			$hideform = true;
		}
	}
...
if(empty($_POST['path'])){ $_POST['path'] = "http://hrwcms.com/cms/"; }
	...
	<p><strong>MySQL / Site Path</strong></p>\n";
		...
		echo "<p>S'il vous pla&icirc;t entrer votre base de donn&eacute;es MySQL, les permission ci-dessous, et le chemin de votre site. Si vous n'&ecirc;tes pas s&ucirc;r des
param&egrave;tres MySQL, s'il vous pla&icirc;t contacter votre h&eacute;bergeur.</p>
	<table class=\"form-table\">
		<tr>
			<th scope=\"row\">Nom de la base de donn&eacute;es</th>
			<td><input name=\"name\" type=\"text\" size=\"25\" value=\"".$_POST['name']."\" /></td>
			<td>Nom de la BDD de HRW Cms©</td>
		</tr>
		<tr>
			<th scope=\"row\">Nom d'utilisateur</th>
			<td><input name=\"username\" type=\"text\" size=\"25\" value=\"".$_POST['username']."\" /></td>
			<td>Votre nom d'utilisateur MySQL, habituellement 'root'.</td>
		</tr>
		<tr>
			<th scope=\"row\">Mot de passe</th>
			<td><input name=\"pass\" type=\"text\" size=\"25\" value=\"".$_POST['pass']."\" /></td>
			<td>Le mot de passe li&eacute; &agrave; l'utilisateur de MySQL.</td>
		</tr>
		<tr>
			<th scope=\"row\">h&ocirc;te de la base de donn&eacute;es</th>
			<td><input name=\"host\" type=\"text\" size=\"25\" value=\"".$_POST['host']."\" /></td>
			<td>90% de chance, vous n'aurez pas besoin de changer cette valeur.</td>
		</tr>
		<tr>
			<th scope=\"row\">Site Path</th>
			<td><input name=\"path\" type=\"text\" size=\"25\" value=\"".$_POST['path']."\" /></td>
			<td>Le chemin d'acc&egrave;s complet au r&eacute;pertoire votre HRW Cms©, se terminant avec une barre oblique (/).</td>
		</tr>
	</table>
	<p>Choisissez votre syst&egrave;me de salage pour le cryptage des mots de passe, vous ne pouvez pas changer cela plus tard ou de mauvaises choses qui se passeront. ( Laisser Comme Sa Par défaut ).</p>
	...
	<h2 class=\"step\">
	<input name=\"submit\" type=\"submit\" value=\"Envoyer\" class=\"button\" />
	</h2>
</form>";
	} else {
		echo "<p><strong>Connect&eacute; avec succ&egrave;s &agrave; la base de
donn&eacute;es MySQL.</strong></p>\n";
		echo "<p>Le fichier de configuration a &eacute;t&eacute; configurer et la connexion MySQL a &eacute;t&eacute; mis en place avec succ&egrave;s. appuyer sur 'Lancer installation' pour commencer le processus d'installation automatis&eacute;e,<br /><br /><strong>Avertissement:</strong><br /> Cela permettra de reconstituer une base de donn&eacute;es existante HoloCMS tables et en ajouter de nouvelles le cas &eacute;ch&eacute;ant, et donc aussi les
r&egrave;initialisiez. Vous aurez &agrave; cr&eacute;er une opportunit&eacute; de l'utilisateur admin sur le c&ocirc;t&eacute; pendant le processus d'installation<strong></strong></p>";
		echo "<p><a href=\"install.php?step=2\" class=\"button\">Lancer Installation</a>\n&nbsp;<a href=\"install.php?step=1\" class=\"button\">Retour</a></p>\n";
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////
	...
	// so we should be safe to do anything we please here!
		...
		echo "<p>L'installation est maintenant en cours, ce qui devrait prendre seulement une ou deux secondes sur un serveur MySQL moyen pour terminer.</p>\n";
  ...
  PRIMARY KEY  (`id`)
	...
	mysql_query("CREATE TABLE `cms_campaigns` (
...
mysql_query("CREATE TABLE IF NOT EXISTS `users_vipclub` (
...
('4574', 'xmas_gifts_bg2', '4', '0', 'xmas_gifts_bg2', '2', '1', '248');") or die(mysql_error());
		...
		echo "<p>Les tableaux ont &eacute;t&eacute; cr&eacute;&eacute;s, modifi&eacute;s tronqu&eacute; et en cas de besoin, les donn&eacute;es requises a &eacute;t&eacute;
ins&eacute;r&eacute;. Votre site est techniquement pr&ecirc;te &agrave; l'emploi aujourd'hui, mais nous allons prendre un moment pour mettre en place un compte d'administrateur pour vous.</p>";
		echo "<p><a href=\"install.php?step=3\" class=\"button\">Proc&eacute;der</a>\n";
...
///////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(isset($_POST['username'])){
		...
		if($password !== $password2){
			...
			if($already_exists > 0){
				$fail = true;
				$error = "Nom d'utilisateur d&eacute;j&agrave; en utilisation!";
			} else {
				$password = HoloHash($password, $name);
				$scredits = 5000;
				$dob = "2-7-1995";
				$figure = "hr-802-61.hd-190-9.ch-260-64.lg-270-64.sh-300-92.fa-1207-.ha-1015-.wa-2005";
				$gender = "M";
	...
	if(!isset($_POST['email'])){ $_POST['email'] = "email@hrwcms.org"; }

	if(!isset($fail) || $fail == true || isset($error)){
		if(isset($error)){ echo "<p><strong>".$error."</strong></p>\n"; }
		echo "<p>Bon, nous avons termin&eacute; l'installation. Maintenant, nous allons prendre un moment pour mettre en place un compte d'administrateur pour vous. Remplissez le formulaire ci-dessous, et un compte administrateur sera automatiquement cr&eacute;&eacute; pour vous.</p>";
		echo "<p><a href='install.php?step=3&do=skip'>Ignorer cette &eacute;tape</a></p>";
		echo "<form method='post' action='install.php?step=3&do=submit'>";
		echo "	<table class=\"form-table\">
		<tr>
			<th scope=\"row\">Nom d'utilisateur</th>
			<td><input name=\"username\" type=\"text\" size=\"25\" value=\"".$_POST['username']."\" /></td>
			<td>Le nom d'utilisateur que vous utilisez pour vous connecter.</td>
		</tr>
		<tr>
			<th scope=\"row\">Mot de passe</th>
			<td><input name=\"password\" type=\"password\" size=\"25\" value=\"\" /></td>
			<td>Le mot de passe que vous utilisez pour vous connecter avec votre nom d'utilisateur.</td>
		</tr>
		<tr>
			<th scope=\"row\">Confirmer le mot de passe</th>
			<td><input name=\"password-retyped\" type=\"password\" size=\"25\" value=\"\" /></td>
			<td>re-taper votre mot de passe ici.</td>
		</tr>
		<tr>
	...
	<input name=\"submit\" type=\"submit\" value=\"Envoyer\" class=\"button\" />
		...
		echo "<p><h1>Installation termin&eacute;e</h1></p>";
		echo "<p>HRW Cms© a &eacute;t&eacute; install&eacute; avec succ&egrave;s, et un utilisateur admin a &egrave;t&egrave; cr&egrave;&egrave; ! Vous pouve&eacute;z maintenant allez <a href='index.php'>sur votre nouveau site</a> ou allez dans <a href='./housekeeping/index.php?p=login&x=installer'>Admin CP (Housekeeping)</a>.</p>";
		echo "<p><strong>N'oubliez pas de supprimer install.php et upgrade.php, ou vous ne serez pas en mesure d'utiliser votre site.</strong></p>";
		exit;
	}
}
