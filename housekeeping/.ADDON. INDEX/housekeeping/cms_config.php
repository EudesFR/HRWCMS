...
if(isset($_POST['sitename'])){
	...
	if(!empty($sitename) && !empty($shortname) && !empty($language)){
		...
		mysql_query("UPDATE cms_system SET sitename = '".$sitename."', shortname = '".$shortname."', enable_sso = '".$enable_sso."', frontpage = '".$frontpage."', language = '".$language."' , start_credits='".$start_credits."', analytics = '".$analytics."' LIMIT 1") or die(mysql_error());
...
<tr>
<td class='tablerow1'  width='40%'  valign='middle'><b>Index Page</b><div class='graytext'>OLD = "THE OLD HABBO PAGE" NEW = "THE NEW HABBO PAGE"</div></td>
<td class='tablerow2'  width='60%'  valign='middle'><select name='frontpage'  class='dropdown'>
									<option value='old'>Old</option>
									<option value='new' <?php if(FetchCMSSetting('frontpage') == "New"){ echo "selected='selected'"; } ?>>New</option>
								    </select>
</td>
</tr>
